...
```shell
cd oneAPI-samples/DirectProgramming/DPC++/DenseLinearAlgebra/matix_mul/
```
...
+ build.sh.o789510:
  ...
  ```
  ######################################################################## #      Date:           Thu 04 Feb 2021 09:08:50 AM PST #    Job ID:           789510.v-qsvr-1.aidevcloud #      User:           u59998 # Resources:           neednodes=1:gpu:ppn=2,nodes=1:gpu:ppn=2,walltime=06:00:00 ########################################################################  :: WARNING: setvars.sh has already been run. Skipping re-execution.
     To force a re-execution of setvars.sh, use the '--force' option.
     Using '--force' can result in excessive use of your environment variables.
  rm -rf matrix_mul_dpc matrix_mul_omp dpcpp -std=c++17 -g -o matrix_mul_dpc src/matrix_mul_dpcpp.cpp  ######################################################################## # End of output for job 789510.v-qsvr-1.aidevcloud # Date: Thu 04 Feb 2021 09:09:01 AM PST ########################################################################
  ```

+ run.sh.o789513:

  ```
  ######################################################################## #      Date:           Thu 04 Feb 2021 09:09:23 AM PST #    Job ID:           789513.v-qsvr-1.aidevcloud #      User:           u59998 # Resources:           neednodes=1:gpu:ppn=2,nodes=1:gpu:ppn=2,walltime=06:00:00 ########################################################################  :: WARNING: setvars.sh has already been run. Skipping re-execution.
     To force a re-execution of setvars.sh, use the '--force' option.
     Using '--force' can result in excessive use of your environment variables.
  ./matrix_mul_dpc Device: Intel(R) Core(TM) i9-10920X CPU @ 3.50GHz Problem size: c(150,600) = a(150,300) * b(300,600) Result of matrix multiplication using DPC++: Success - The results are correct!
   ######################################################################## # End of output for job 789513.v-qsvr-1.aidevcloud # Date: Thu 04 Feb 2021 09:09:28 AM PST ########################################################################
  ```

可以看到，该样例执行了大小为（150，300）的矩阵与大小为（300，600）的矩阵乘法，计算被分配到Intel(R) Core(TM) i9-10920X CPU @ 3.50GHz计算节点上执行。
...
可以将for循环定义的操作（应该是任何计算的操作）作为kernel进行并行操作

需要使用到的数据应用accessor在每个提交的任务中定义。能否提前定义accessor进行复用？似乎不能？accessor是为了提供用户打算使用数据的方式，方便runtime时组织数据，每个任务互相独立，应该不能复用。或者accessor只能在任务内进行定义？
...
说明accessor可以进行复用，不过需要在任务内部通过handler进行引用。